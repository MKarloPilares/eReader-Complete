...
const Tag_Assessment = ({setCurrentPage, setQuery, setBackPage}) => {
  ...
  >
    <Container>
        ...
        <Row>
          ...
          <Card.Img variant="top"  style={{borderBottom: 'gray 1px solid'}} src={loadImage(mess.AssImg.data)}/> 
          <Card.Body>
          ...
          <Col class="col-md-4">
          <Card border="success" style={{ width: '30rem' }}> 
          <Card.Img variant="top" style={{borderBottom: 'gray 1px solid'}} src={loadImage(post[index+1].AssImg.data)}/> 
          <Card.Body>
            <Card.Title>{post[index+1].AssName}</Card.Title>
            <Card.Text>{post[index+1].AssDesc} </Card.Text>
            <Button variant="success" onClick={() => {setCurrentPage("OralAssessment"); setQuery(post[index+1].AssName); setBackPage("Tag_Assessments")}} size='lg'style={{position: "relative", left: "350px", fontWeight: 'bold'}}>START</Button>
          </Card.Body>
          </Card>
          </Col>)}
      </Row> ) : (<p></p>)}
